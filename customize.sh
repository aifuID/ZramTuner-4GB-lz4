...
msg "=================================="
msg "   ZRAM 4GB+lz4 - DIRECT INSTALL"
msg "=================================="
for F in "$MDIR"/*.txt; do
  [ -f "$F" ] || continue
  tr -d '\r' < "$F" | while IFS= read -r L; do msg "$L"; done
done
msg "   ZRAM 4GB+lz4 EDITION | (c) @aifu-ID x AI"
msg "kernel support:"
...
versionCode=48
author=aifu-ID
description=(c) @aifu-ID x AI - zram 4GB+lz4
PROP
...
msg "now: $(cat $SYS/disksize 2>/dev/null) | $(cat $SYS/comp_algorithm 2>/dev/null)"
msg "=================================="
